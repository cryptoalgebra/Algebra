...
describe('unit/Deposits', () => {
  ...
  describe('#onERC721Received', () => {
    ...
    describe('on successful transfer with staking data', () => {
      ...
      it('properly stakes the deposit in the select incentive', async () => {
        ...
        expect((await context.staker.deposits(tokenId))._tokenId).to.equal(1)
    ...
    describe('on invalid call', async () => {
      it('reverts when called by contract other than Algebra nonfungiblePositionManager', async () => {
        ...
        ).to.be.revertedWith('AlgebraStaker::onERC721Received: not a  Algebra nft')
      })
    ...
    describe('fails if', () => {
      it('you are withdrawing a token that is not yours', async () => {
        ...
        await expect(context.staker.connect(notOwner).withdrawToken(tokenId, notOwner.address, '0x')).to.revertedWith(
          'AlgebraStaker::withdrawToken: only owner can withdraw token'
        )
      })
      ...
      it('number of stakes is not 0', async () => {
        ...
        await expect(subject(tokenId, lpUser0.address)).to.revertedWith(
          'AlgebraStaker::withdrawToken: cannot withdraw token while staked'
        )
      })
    })
  })
  ...
  //       'AlgebraStaker::transferDeposit: can only be called by deposit owner'
  //     )
  //   })
  ...
  //       'AlgebraStaker::transferDeposit: invalid transfer recipient'
  //     )
  //   })
