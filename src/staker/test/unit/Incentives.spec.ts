...
describe('unit/Incentives', async () => {
  ...
  describe('#createIncentive', () => {
    ...
    describe('works and', () => {
      ...
      it('adds to existing incentives', async () => {
        ...
        const { numberOfStakes } = await context.staker.incentives(
          incentiveId
        )
      ...
      it('does not override the existing numberOfStakes', async () => {
        ...
        ;({ numberOfStakes } = await context.staker
          .connect(actors.lpUser0())
    ...
    describe('fails when', () => {
      describe('invalid timestamps', () => {
        ...
        it('end time is before start time', async () => {
          ...
          await expect(subject(params)).to.be.revertedWith(
            'AlgebraStaker::createIncentive: start time must be before end time'
          )
        })
  ...
  //         'AlgebraStaker::endIncentive: cannot end incentive before end time'
  //       )
  //     })
  //
  ...
  //         })
  //       ).to.be.revertedWith('AlgebraStaker::endIncentive: no refund available')
  //     })
  //
  ...
  //       await Time.set(timestamps.endTime + 1)
  //       await expect(subject({})).to.be.revertedWith(
  //         'AlgebraStaker::endIncentive: cannot end incentive while deposits are staked'
  //       )
  //     })
  //   })
  // })
