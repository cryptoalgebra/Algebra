  ...
  describe('#stakeToken', () => {
    ...
    describe('works and', () => {
      ...
      it('sets the stake struct properly', async () => {
        ...
        const depositStakesBefore = (await context.staker.deposits(tokenId))._tokenId
        await subject(tokenId, lpUser0)
        const stakeAfter = await context.staker.stakes(tokenId, incentiveId)
        const depositStakesAfter = (await context.staker.deposits(tokenId))._tokenId

        expect(stakeBefore).to.eq(0)
        expect(depositStakesBefore).to.eq(0)
        expect(stakeAfter).to.eq(liquidity)
        expect(depositStakesAfter).to.eq(1)
      })

      it('increments the number of stakes on the deposit', async () => {
        const nStakesBefore: BigNumber = (await context.staker.deposits(tokenId))._tokenId
        await subject(tokenId, lpUser0)

        expect((await context.staker.deposits(tokenId))._tokenId).to.eq(nStakesBefore.add(1))
      })
    ...
    describe('fails when', () => {
      it('deposit is already staked in the incentive', async () => {
        ...
        await subject(tokenId, lpUser0)
        await expect(subject(tokenId, lpUser0)).to.be.revertedWith('AlgebraStaker::stakeToken: already staked')
      })
      ...
      it('you are not the owner of the deposit', async () => {
        ...
        await expect(subject(tokenId, actors.lpUser2())).to.be.revertedWith(
          'AlgebraStaker::stakeToken: only owner can stake token'
        )
      })
      ...
      it('has 0 liquidity in the position', async () => {
        ...
        await expect(subject(tokenId2, lpUser0)).to.be.revertedWith(
          'AlgebraStaker::stakeToken: cannot stake token with 0 liquidity'
        )
      })
      ...
      it('token id is for a different pool than the incentive', async () => {
        ...
        ).to.be.revertedWith('AlgebraStaker::stakeToken: token pool is not the incentive pool')
      })
      ...
      it('incentive key does not exist', async () => {
        ...
        ).to.be.revertedWith('AlgebraStaker::stakeToken: non-existent incentive')
      })

      it('is past the end time', async () => {
        await Time.set(timestamps.endTime + 100)
        await expect(subject(tokenId, lpUser0)).to.be.revertedWith('AlgebraStaker::stakeToken: incentive has already started')
      })
  ...
  describe('#getRewardInfo', () => {
    ...
    it('reverts if stake does not exist', async () => {
      ...
      await expect(context.staker.connect(lpUser0).getRewardInfo(stakeIncentiveKey, '100')).to.be.revertedWith(
        'AlgebraStaker::getRewardInfo: stake does not exist'
      )
    })
  })
  ...
  describe('#unstakeToken', () => {
    ...
    describe('before end time', () => {
      it('cannot unstake', async() => {
        ...
        await expect(context.staker.connect(actors.lpUser0()).unstakeToken(
          {
            refundee: incentiveCreator.address,
            pool: context.pool01,
            rewardToken: context.rewardToken.address,
          ...
          tokenId
        )).to.be.revertedWith('AlgebraStaker::unstakeToken: cannot unstake before end time')
      })
    })
    ...
    describe('works and', () => {
      it('decrements deposit numberOfStakes by 1', async () => {
        const { _tokenId: stakesPre } = await context.staker.deposits(tokenId)
        await subject(lpUser0)
        const { _tokenId: stakesPost } = await context.staker.deposits(tokenId)
      ...
      it('emits an unstaked event', async () => {
        await expect(subject(lpUser0)).to.emit(context.staker, 'TokenUnstaked').withArgs(
            tokenId,
            incentiveId,
