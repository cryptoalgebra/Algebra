...
async function main() {
  ...
  const TickLens = await TickLensFactory.deploy({ gasLimit: "0x1000000" })


  // arg1 factory address
  // arg2 wnative address
  const QuoterFactory = await hre.ethers.getContractFactory("Quoter");
  const Quoter = await QuoterFactory.deploy(deploysData.factory, WNativeTokenAddress, deploysData.poolDeployer, { gasLimit: "0x1000000" });
  ...
  const SwapRouter = await SwapRouterFactory.deploy(deploysData.factory, WNativeTokenAddress, deploysData.poolDeployer, { gasLimit: "0x1000000" });
  ...
  const NFTDescriptor = await NFTDescriptorFactory.deploy({ gasLimit: "0x1000000" })
    ...
    const NonfungibleTokenPositionDescriptor = await NonfungibleTokenPositionDescriptorFactory.deploy(WNativeTokenAddress, { gasLimit: "0x1000000" })
  ...
  const Proxy = await ProxyFactory.deploy(NonfungibleTokenPositionDescriptor.target, ProxyAdmin, "0x", { gasLimit: "0x1000000" }) 
  ...
  const NonfungiblePositionManager = await NonfungiblePositionManagerFactory.deploy(deploysData.factory, WNativeTokenAddress, Proxy.target, deploysData.poolDeployer, { gasLimit: "0x1000000" });
  ...
  const V3Migrator = await V3MigratorFactory.deploy(deploysData.factory, WNativeTokenAddress, NonfungiblePositionManager.target,deploysData.poolDeployer, { gasLimit: "0x1000000" });
  


  const AlgebraInterfaceMulticallFactory = await hre.ethers.getContractFactory('AlgebraInterfaceMulticall')
  const AlgebraInterfaceMulticall = await AlgebraInterfaceMulticallFactory.deploy({ gasLimit: "0x1000000" })
