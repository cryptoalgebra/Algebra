...
async function main() {
  ...
  await NFTDescriptor.waitForDeployment()
    ...
    const NonfungibleTokenPositionDescriptor = await NonfungibleTokenPositionDescriptorFactory.deploy(WNativeTokenAddress,"AA",[])
  ...
  const NonfungiblePositionManager = await NonfungiblePositionManagerFactory.deploy(deploysData.factory, WNativeTokenAddress, Proxy.target, deploysData.poolDeployer );
  ...
  const V3Migrator = await V3MigratorFactory.deploy(deploysData.factory, WNativeTokenAddress, NonfungiblePositionManager.target,deploysData.poolDeployer );
