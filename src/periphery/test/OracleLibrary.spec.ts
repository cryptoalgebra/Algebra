...
describe('DataStorageLibrary', () => {
  ...
  describe('#getQuoteAtTick', () => {
    ...
    it('token0: returns correct value when tick = 0', async () => {
      const quoteAmount = await dataStorage.getQuoteAtTick(0n, expandTo18Decimals(1), tokens[0], tokens[1]);
    ...
    it('token1: returns correct value when tick = 0', async () => {
      const quoteAmount = await dataStorage.getQuoteAtTick(0n, expandTo18Decimals(1), tokens[1], tokens[0]);

      expect(quoteAmount).to.equal(expandTo18Decimals(1));
    });

    it('token0: returns correct value when at min tick | 0 < sqrtRatioX96 <= type(uint128).max', async () => {
      const quoteAmount = await dataStorage.getQuoteAtTick(-887272n, 2n ** 128n - 1n, tokens[0], tokens[1]);
      expect(quoteAmount).to.equal(1n);
    });

    it('token1: returns correct value when at min tick | 0 < sqrtRatioX96 <= type(uint128).max', async () => {
      const quoteAmount = await dataStorage.getQuoteAtTick(-887272n, 2n ** 128n - 1n, tokens[1], tokens[0]);
    ...
    it('gas test [ @skip-on-coverage ]', async () => {
      await snapshotGasCost(dataStorage.getGasCostOfGetQuoteAtTick(10n, expandTo18Decimals(1), tokens[0], tokens[1]));
    });
  });
});
