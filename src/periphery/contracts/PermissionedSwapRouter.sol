...
/// @title Algebra Integral 1.2.2 Permissioned Swap Router
/// @notice Router for stateless execution of swaps against Algebra, which additionally reports the real
/// end-user via IMsgSender, so it can be trusted by permissioned-pool plugins that need to see past this
/// router as the immediate caller
/// @dev A fork of SwapRouter with `reportSender` added to every entry point that can reach a pool hook.
/// Only pools that have explicitly registered this router's address as trusted (via the plugin's
/// `setRouterAllowed`) will act on its `msgSender()` report; all other pools treat this router exactly
/// like the vanilla SwapRouter
/// @dev Credit to Uniswap Labs under GPL-2.0-or-later license:
/// https://github.com/Uniswap/v3-periphery
contract PermissionedSwapRouter is
    ISwapRouter,
    ...
    SelfPermit
{
    ...
    ) external payable override checkDeadline(params.deadline) reportSender returns (uint256 amountOut) {
    ...
    ) external payable override checkDeadline(params.deadline) reportSender returns (uint256 amountIn) {
