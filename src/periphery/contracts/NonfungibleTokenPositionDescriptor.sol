...
contract NonfungibleTokenPositionDescriptor is INonfungibleTokenPositionDescriptor {
    ...
    function tokenURI(
        ...
        (, , address token0, address token1, address deployer, int24 tickLower, int24 tickUpper, , , , , ) = positionManager.positions(
            tokenId
        );
        ...
        IAlgebraPool pool = IAlgebraPool(
            PoolAddress.computeAddress(
                ...
                PoolAddress.PoolKey({token0: token0, token1: token1, deployer: deployer})
            )
        );
