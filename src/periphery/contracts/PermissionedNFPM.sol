...
/// @title Algebra Integral 1.2.2 Permissioned NFT positions
/// @notice Wraps Algebra positions in the ERC721 non-fungible token interface, and additionally reports
/// the real end-user via IMsgSender, so it can be trusted by permissioned-pool plugins that need to see
/// past this contract as the immediate caller
/// @dev A fork of NonfungiblePositionManager with `reportSender` added to every entry point that can
/// reach a pool hook. Only pools that have explicitly registered this contract's address as trusted (via
/// the plugin's `setRouterAllowed`) will act on its `msgSender()` report; all other pools treat this
/// contract exactly like the vanilla NonfungiblePositionManager
/// @dev Credit to Uniswap Labs under GPL-2.0-or-later license:
/// https://github.com/Uniswap/v3-periphery
contract PermissionedNFPM is
