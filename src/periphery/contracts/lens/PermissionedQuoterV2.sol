...
import '../base/PeripheryMsgSender.sol';
import '../libraries/Path.sol';
import '../libraries/PoolAddress.sol';
import '../libraries/CallbackValidation.sol';
import '../libraries/PoolTicksCounter.sol';

/// @title Algebra Integral 1.2.2 Permissioned QuoterV2
/// @notice Allows getting the expected amount out or amount in for a given swap without executing the swap,
/// and additionally reports the real end-user via IMsgSender, so it can be trusted by permissioned-pool
/// plugins that need to see past this contract as the immediate caller
/// @dev These functions are not gas efficient and should _not_ be called on chain. Instead, optimistically execute
/// the swap and check the amounts in the callback.
/// @dev A fork of QuoterV2 with `reportSender` added to every entry point that can reach a pool hook. Only
/// pools that have explicitly registered this contract's address as trusted (via the plugin's
/// `setRouterAllowed`) will act on its `msgSender()` report; all other pools treat this contract exactly
/// like the vanilla QuoterV2
/// @dev Credit to Uniswap Labs under GPL-2.0-or-later license:
/// https://github.com/Uniswap/v3-periphery
contract PermissionedQuoterV2 is IQuoterV2, IAlgebraSwapCallback, PeripheryImmutableState, PeripheryMsgSender {
    ...
    /// @dev Holds the actual exact-input single-pool quote logic, kept separate from `quoteExactInputSingle`
    /// so that `quoteExactInput` can reuse it without nesting the `reportSender` modifier, which is not
    /// reentrant-safe
    function quoteExactInputSingleInternal(
    ...
    {
        return quoteExactInputSingleInternal(params);
    }
        ...
        )
    {
        ...
        while (true) {
            ...
            (
                ...
                _gasEstimate,
                feeList[i]
            ) = quoteExactInputSingleInternal(params);
    ...
    /// @dev Holds the actual exact-output single-pool quote logic, kept separate from `quoteExactOutputSingle`
    /// so that `quoteExactOutput` can reuse it without nesting the `reportSender` modifier, which is not
    /// reentrant-safe
    function quoteExactOutputSingleInternal(
    ...
    {
        return quoteExactOutputSingleInternal(params);
    }
        ...
        )
    {
        ...
        while (true) {
            ...
            (
                ...
                _gasEstimate,
                feeList[i]
            ) = quoteExactOutputSingleInternal(params);
