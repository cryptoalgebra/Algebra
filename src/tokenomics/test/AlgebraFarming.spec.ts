...
describe('AlgebraFarming', () => {
  ...
  describe('minimal position width', async () => {
    ...
    it('cannot enter with too narrow position', async () => {
      ...
      await expect(
        context.farmingCenter.connect(lpUser3).enterFarming(incentiveResultToFarmAdapter(createIncentiveResult), tokenId, 0, LIMIT_FARMING)
      ).to.be.revertedWithCustomError(context.farming as AlgebraLimitFarming, 'positionIsTooNarrow')
    })
    ...
    it('max range can be used as minimal allowed', async () => {
      ...
      await expect(
        context.farmingCenter.connect(lpUser3).enterFarming(incentiveResultToFarmAdapter(createIncentiveResult), tokenId, 0, ETERNAL_FARMING)
      ).to.be.revertedWithCustomError(context.eternalFarming as AlgebraEternalFarming, 'positionIsTooNarrow')

      await expect(
        context.farmingCenter.connect(lpUser3).enterFarming(incentiveResultToFarmAdapter(createIncentiveResult), tokenIdCorrect, 0, ETERNAL_FARMING)
      ).to.be.not.reverted
    })
  })
