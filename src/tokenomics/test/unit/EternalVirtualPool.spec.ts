...
describe('unit/EternalVirtualPool', () => {
  ...
  it('cannot call onlyFarming methods as not farming', async() => {
    ...
    await expect(virtualPool.applyLiquidityDeltaToPosition(
      100, 110, 100, 100
  ...
  describe('#applyLiquidityDeltaToPosition', async() => {
    ...
    it('cannot overflow liquidity if out-of range', async () => {
      await virtualPool.connect(pseudoFarming).applyLiquidityDeltaToPosition(
        -100,
        100,
        191757638537527648490752896198552n,
        -101
      );
      ...
      await expect(virtualPool.connect(pseudoFarming).applyLiquidityDeltaToPosition(
        -100,
        100,
        2n,
        -101
      )).to.be.revertedWithCustomError(virtualPool, 'liquidityOverflow');
    })
