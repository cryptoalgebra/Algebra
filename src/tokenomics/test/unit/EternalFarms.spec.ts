...
describe('unit/EternalFarms', () => {
  ...
  describe('#onlyFarmingCenter ', () => {
    ...
    it('reverts if not farmingCenter', async ()=> {
      ...
      expect(context.eternalFarming.connect(actors.farmingDeployer()).enterFarming(
        dummyKey,
        1
      ))
      .to.be.revertedWithoutReason;

      expect(context.eternalFarming.connect(actors.farmingDeployer()).exitFarming(
        dummyKey,
        1,
        ZERO_ADDRESS
      ))
      ...
      expect(context.eternalFarming.connect(actors.farmingDeployer()).collectRewards(
        dummyKey,
        1,
        ZERO_ADDRESS
      ))
  ...
  describe('#onlyIncentiveMaker', () => {
    ...
    it('reverts if not incentiveMaker', async ()=> {
      expect(context.eternalFarming.connect(actors.farmingDeployer()).createEternalFarming(
        dummyKey,
        {
          reward: 100,
          bonusReward: 100,
      ...
      expect(context.eternalFarming.connect(actors.farmingDeployer()).deactivateIncentive(
        dummyKey
      ))
      .to.be.revertedWithoutReason;

      expect(context.eternalFarming.connect(actors.farmingDeployer()).setRates(
        dummyKey,
        10,
        10
      ))
  ...
  describe('#enterFarming', () => {
    ...
    describe('fails when', () => {
      ...
      it('trying to forcefully enter twice', async () => {
        ...
        await expect(context.eternalFarming.connect(lpUser0).enterFarming(
          {
            pool: context.pool01,
            rewardToken: context.rewardToken,
            bonusRewardToken: context.bonusRewardToken,
  ...
  xdescribe('swap gas [ @skip-on-coverage ]', async () => {
    ...
    it('swap with second cross', async () => {
      let totalReward = BigInt('9000000000000')
      let bonusReward = BigInt('9000000000000')
  ...
  describe('#decreaseRewards', () => {
    ...
    it('can decrease rewards before start', async () => {
      ...
      .to.emit(context.rewardToken, 'Transfer').withArgs(await context.eternalFarming.getAddress(), factoryOwner.address, 100)
      .to.emit(context.bonusRewardToken, 'Transfer').withArgs(await context.eternalFarming.getAddress(), factoryOwner.address, 100)
  ...
  describe('#deactivate incentive', () => {
    ...
    it('cannot deactivate twice', async () => {
      await context.eternalFarming.connect(incentiveCreator).deactivateIncentive(incentiveKey);
      await expect(context.eternalFarming.connect(incentiveCreator).deactivateIncentive(incentiveKey)).to.be.revertedWithCustomError(context.eternalFarming, 'incentiveStopped');
    })

    it('cannot deactivate nonexistent incentive', async () => {
      const invalidKey = { ...incentiveKey };
      invalidKey.nonce = 999;
      await expect(context.eternalFarming.connect(incentiveCreator).deactivateIncentive(invalidKey)).to.be.revertedWithCustomError(context.eternalFarming, 'incentiveNotExist');
    })
  ...
  describe('#claimReward', () => {
    ...
    beforeEach('setup', async () => {
      ...
      subject = (_token: string | TestERC20, _to: string, _amount: bigint) => context.eternalFarming.connect(lpUser0).claimReward(_token, _to, _amount)
    })
    ...
    describe('when requesting the full amount', () => {
      ...
      xit('has gas cost [ @skip-on-coverage ]', async () => await snapshotGasCost(subject(await context.rewardToken.getAddress(), lpUser0.address, BN('0'))))
    ...
    describe('when requesting a nonzero amount', () => {
      ...
      it('reverts if transfer to zero', async () => {
        const { rewardToken } = context
        claimable = await context.eternalFarming.rewards(lpUser0.address, rewardToken)
        await expect(subject(rewardToken, ZERO_ADDRESS, claimable)).to.be.revertedWithCustomError(
          context.eternalFarming,
          'claimToZeroAddress'
        );
      })
      ...
      it('not emit event if nothing to claim', async () => {
        const { rewardToken } = context
        await expect(context.eternalFarming.connect(actors.lpUser2()).claimReward(rewardToken, actors.lpUser2().address, 100n))
  ...
  describe('#collectRewards', async () => {
    ...
    it('cannot collect from nonexistent farm', async () => {
      ...
      await expect(context.eternalFarming.connect(lpUser0).collectRewards(
        {
          rewardToken: await context.rewardToken.getAddress(),
          bonusRewardToken: await context.bonusRewardToken.getAddress(),
          pool: context.pool01,
    ...
    describe('after end time', () => {
      ...
      describe('works and', () => {
        it('emits an exitFarmingd event', async () => {
          await expect(subject(lpUser0))
            ...
            .withArgs(tokenId, incentiveId, await context.rewardToken.getAddress(), await context.bonusRewardToken.getAddress(), lpUser0.address, 9999n, 199n)
        })
      ...
      it('can exit without rewards', async () => {
        await expect(
          ...
          .to.emit(context.eternalFarming, 'FarmEnded')
          .withArgs(tokenIdOut, incentiveId, await context.rewardToken.getAddress(), await context.bonusRewardToken.getAddress(), lpUser0.address, BN('0'), BN('0'))
      })
      ...
      it('cannot exit from nonexistent farming', async () => {
        ...
        await expect(context.eternalFarming.connect(lpUser0).exitFarming(
          {
            pool: context.pool12,
            rewardToken: await context.rewardToken.getAddress(),
            bonusRewardToken: await context.bonusRewardToken.getAddress(),
