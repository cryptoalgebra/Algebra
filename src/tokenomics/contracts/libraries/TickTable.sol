...
library TickTable {
    ...
    function toggleTick(mapping(int16 => uint256) storage self, int24 tick) internal {
        require(tick % VirtualPoolConstants.TICK_SPACING == 0, 'tick is not spaced'); // ensure that the tick is spaced
        tick /= VirtualPoolConstants.TICK_SPACING; // compress tick
        int16 rowNumber;
        uint8 bitNumber;
    ...
    function nextTickInTheSameRow(
        ...
        {
            int24 tickSpacing = VirtualPoolConstants.TICK_SPACING;
    ...
    function uncompressAndBoundTick(int24 tick) private pure returns (int24 boundedTick) {
        boundedTick = tick * VirtualPoolConstants.TICK_SPACING;
