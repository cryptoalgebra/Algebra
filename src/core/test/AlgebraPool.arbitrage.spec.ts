            ...
            describe(description, () => {
              ...
              it('sandwiched with swap to execution price then mint max liquidity/target/burn max liquidity', async () => {
                ...
                const mintReceipt = await (
                  await mint(wallet.address, bottomTick, topTick, BigNumber.from("191757638537527648490752896198553"))
                ).wait()
                // sub the mint costs
                ...
                const { amount0: amount0Burn, amount1: amount1Burn } = await pool.callStatic.burn(
                  bottomTick,
                  topTick,
                  BigNumber.from("191757638537527648490752896198553")
                )
                await pool.burn(bottomTick, topTick, BigNumber.from("191757638537527648490752896198553"))
