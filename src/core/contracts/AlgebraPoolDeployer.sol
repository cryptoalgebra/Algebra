...
contract AlgebraPoolDeployer is IAlgebraPoolDeployer {
  ...
  function getDeployParameters() external view override returns (address _plugin, address _factory, address _token0, address _token1, address _pluginDeployer) {
    (_plugin, _token0, _token1, _pluginDeployer) = _readFromCache();
    _factory = factory;
  }
  ...
  function deploy(address plugin, address token0, address token1, address deployer) external override returns (address pool) {
    ...
    _writeToCache(plugin, token0, token1, deployer);
    ...
    (cache0, cache1, cache2) = (bytes32(0), bytes32(0), bytes32(0)); // clear cache after deployment
  }

  /// @notice densely packs three addresses into two storage slots
  function _writeToCache(address plugin, address token0, address token1, address pluginDeployer) private {
    assembly {
  ...
  function _readFromCache() private view returns (address plugin, address token0, address token1, address pluginDeployer) {
