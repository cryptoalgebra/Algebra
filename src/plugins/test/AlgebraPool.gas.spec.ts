  ...
  for (const communityFee of [0, 60]) {
    describe(communityFee > 0 ? 'fee is on' : 'fee is off', () => {
      ...
      const gasTestFixture = async () => {
        ...
        const mockPluginFactory = (await mockPluginFactoryFactory.deploy(fix.factory.address)) as MockTimeDSFactory
        await mockPluginFactory.deployed();
      
        await mockPluginFactory.createPlugin(pool.address);
      ...
      describe('#swapExact0For1', () => {
        ...
        it('first swap in block, large swap, no initialized crossings, prepaid dataStorage slot', async () => {
          await plugin.prepayTimepointsStorageSlots(3, 1);
          await snapshotGasCost(swapExact0For1(expandTo18Decimals(10), wallet.address))
          expect((await pool.globalState()).tick).to.eq(-35586)
        })
